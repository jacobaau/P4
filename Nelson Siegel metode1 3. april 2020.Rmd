...
```
\section{Nelson-siegel metode 1}
```{r}
...
Definere nelson-siegel nulkuponrentefunktionen

```{r}
parameter_ <- c(-0.002878, 0.004864, -0.025695, 1)
nulkupon_lin <- function(t){
  parameter_[1] + parameter_[2]*((1-exp(-t/parameter_[4]))/(t/parameter_[4])) + parameter_[3]*exp(-t/parameter_[4])
}

```
Skitserer kurven sammen med de observerede punkter
```{r}
{curve(nulkupon_lin, ylim=c(n.renter[1], 0.026), xlim=c(0.01, 32), xlab= "Tid til udløbsdato", ylab = "Nulkuponrente", col="forestgreen", main ="Nulkuponkurven, 3. april 2020", lwd=1.7)
...
legend(9, 0.02, legend = c("Nelson-Siegel metode 1, Theta=1", "Observerede nulkuponrenter"), col = c("forestgreen", "gray30"), lty=c(1,NA), cex=0.8, box.lty=1, pch = c(NA, 1), box.lwd=1, box.col="black")}
```
